...
router.get('/global', optionalAuth, (req, res) => {
  try {
    // 确保所有查询都有默认值
    const projectsResult = db.prepare('SELECT COUNT(*) as count FROM projects').get();
    const total_projects = projectsResult?.count || 0;

    const usersResult = db.prepare('SELECT COUNT(*) as count FROM users').get();
    const total_users = usersResult?.count || 0;

    const engineersResult = db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'engineer' OR certification_status = 'approved'").get();
    const total_engineers = engineersResult?.count || 0;

    const contractsResult = db.prepare('SELECT COUNT(*) as count FROM contracts').get();
    const total_contracts = contractsResult?.count || 0;

    const amountResult = db.prepare("SELECT COALESCE(SUM(amount), 0) as total FROM contracts WHERE status = 'completed'").get();
    const total_amount = amountResult?.total || 0;
    ...
    if (isAdmin) {
      ...
      const total_clients = clientsResult?.count || 0;

      const pendingResult = db.prepare("SELECT COUNT(*) as count FROM users WHERE certification_status = 'pending'").get();
      const pending_certs = pendingResult?.count || 0;

      const activeResult = db.prepare("SELECT COUNT(*) as count FROM contracts WHERE status = 'active'").get();
      const active_contracts = activeResult?.count || 0;

      const biddingResult = db.prepare("SELECT COUNT(*) as count FROM projects WHERE status = 'bidding'").get();
      const bidding_projects = biddingResult?.count || 0;
