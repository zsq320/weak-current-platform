...
- **投标系统**：多步表单投标、多维度评分、排序筛选、评分排名、评分表导出、投标撤回
- **合同管理**：在线签订电子合同、签署前金额调整（议价）
- **消息通知**：站内信通知投标状态变更、点击跳转关联工程/合同、用户间私信
- **工程师主页**：公开主页展示认证状态、企业资质、成交与口碑统计
- **管理后台**：用户管理、数据统计
...
### 商用化能力（V2）
- **资金体系**：流水账本全程留痕、充值订单、提现申请与审核、平台佣金、质保金留存与到期自动释放、平台资金对账自检
- **电子合同**：合同正文模板（可编辑、版本化、内容哈希）、双方密码签署留痕、工程款托管、纠纷仲裁（支持按仲裁结果退款/放款）
- **施工过程**：施工日志、GPS 现场打卡、工程照片、隐蔽工程报验、材料进场、工程量清单 BOQ、阶段/竣工验收与整改、过程数据 CSV 导出
- **售后与合规**：质保工单（可附现场照片）、发票申请与处理、企业认证（资质审核）、评价申诉、用户协议/隐私政策与注册勾选记录
- **协同**：项目双方沟通聊天（留痕）、SSE 实时未读通知、截止日期与里程碑到期自动提醒
- **运营**：管理后台运营中心（提现审核/纠纷仲裁/质保工单/发票/企业/申诉/系统公告/平台设置/对账CSV与资金自检/数据库备份）、结构化日志、历史数据迁移
...
内置 API 回归测试套件（`tests/api-fulltest.js`），覆盖认证、权限、投标、评分、合同结算、任务里程碑、消息、管理后台与安全约束等 122 项断言：

```bash
# 先启动服务，另开终端执行
node tests/api-fulltest.js      # 主流程回归（122项）
node tests/api-commercial.js    # 商用化专项回归（52项）
```
...
### 用户与评价
- `GET /api/users/:id/profile` - 用户公开主页（认证/资质/成交与口碑统计）
- `GET /api/reviews/user/:userId` - 用户收到的评价
- `GET /api/reviews/project/:projectId` - 工程相关评价（公开）

### 投标
- `POST /api/bids` - 提交投标
- `POST /api/bids/:id/withdraw` - 撤回投标（待定且工程招标中）
- `GET /api/bids/project/:projectId/rankings` - 评分排名
- `GET /api/bids/project/:projectId/export` - 导出评分表 CSV

### 施工过程数据导出
- `GET /api/projects/:id/construction/export?type=logs|checkins|materials|boq` - 过程数据 CSV（参与者）
